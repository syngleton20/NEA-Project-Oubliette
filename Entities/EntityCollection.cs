...
namespace NEA_Project_Oubliette.Entities
{
    ...
    internal sealed class EntityCollection
    {
        ...
        public void Add(Entity entity)
        {
            ...
            {
                entities.Add(entity);
                if(Game.Current.CurrentMap.TryGetTile(entity.Position, out Tile tile)) tile.Occupy(entity);
            }
        }
        ...
        public void UpdateAll() { for(int i = 0; i < entities.Count; i++) entities[i]?.Update(); }

        ///<summary>Returns a unique id for a new entity</summary>
        public int AssignId()
        {
            int nextFreeId = 0;
            if(entities.Count <= 0) return nextFreeId; // Returns 0 if there are no entities in this collection

            for(int i = 0; i < entities.Count; i++)
            {
                if(!TryGetEntityById(nextFreeId, out Entity entity)) return nextFreeId; // Returns the nextFreeId if no entity can be found with this ID
                nextFreeId++;
            }

            return nextFreeId; // Returns the nextFreeId if all entities have incremental IDs
        }
